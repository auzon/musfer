package me.oguzhanozer.musfer.userapi.unit;


import me.oguzhanozer.musfer.userapi.controller.UserController;
...
import me.oguzhanozer.musfer.userapi.util.TestUser;
...
import static me.oguzhanozer.musfer.userapi.util.JsonHelper.*;
import static me.oguzhanozer.musfer.userapi.util.TestUserHelper.*;
import static me.oguzhanozer.musfer.userapi.util.UserMatchers.*;
...
public class UserControllerTest {
    ...
    void createUser_shouldReturn201Created_whenUserCreated() throws Exception {
        ...
        when(userService.createUser(argMatches(transientEntityOf(validUser)))).thenReturn(persistentEntityOf(validUser));
    ...
    void createUser_shouldReturn409Conflict_whenEmailAlreadyExists() throws Exception {
        ...
        TestUser validUser = TestUser.WITH_VALID_VALUES;
        when(userMapper.toEntity(argMatches(createRequestOf(validUser)))).thenReturn(transientEntityOf(validUser));
        when(userService.createUser(argMatches(transientEntityOf(validUser)))).thenThrow(EmailAlreadyExistsException.class);
        // act
        mockMvc.perform(post(USERS_ENDPOINT).contentType(APPLICATION_JSON).content(createRequestJsonOf(validUser)))
            // assert
            .andExpectAll(
